...
To build `hazelcast-caching` microservice, run the following command.
...
This file defines two {kube} resources. It defines one statefulset and one service. 
StatefulSet is preferred solution for Hazelcast because it enables controlled scale out/in of your microservices 
for easy data distribution. To learn more about StatefulSet, you can visit Kubernetes documentationhttps://kubernetes.io/docs/tutorials/stateful-application/basic-stateful-set/
...
microservices. Hazelcast requires a read access to autodiscovery other hazelcast members to create distributed cache among 
clusters
...
open your terminal and send http get requests in a loop via `curl` command. 
...
You should see an out like below. You will see that value is `null` because we have not put any data deployment
and responses are coming from different pods.
[source, role="no_copy"]
----
...
Although request has been executed by one kubernetes pod, by querying the data in a loop will show data the data 
...
As you see both pods are returning the same data.
...
Scale the cluster with one more pod and show that you still retrieve the shared data.

[role='command']
```
kubectl scale deployment/name-deployment --replicas=3
```
As you can see, a new pod `hazelcast-caching-statefulset-2` has joined to the cluster and returns the value for the key=1
`
[source, role="no_copy"]
----
{"value":"hazelcast_springboot_openliberty","podName":"hazelcast-caching-statefulset-0"}
...
variable. In the second part, tests submits multipe `/get` requests until to see that podname is different then 
initial pod which handled `/put` request.

In order to run integration tests, you must have a running `hazelcast-caching` microservices 
in minikube environment and wait for all pods to be in the ready state before proceeding further. 
...
you can use `cluster.ip` and `cluster.port` based on your kubernetes cluster.
[role='command']
```
...
As a last step, you ran integration tests against `hazelcast-caching`that is deployed in a {kube} cluster.
